...
self.addEventListener('install', (event) => {
  const assetsToPrecache = [
    ...
    getAbsPath('/assets-manifest.json')
  ];
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    fetch(getAbsPath('/assets-manifest.json?t=') + Date.now())
        ...
        return caches.open(IMAGE_CACHE_NAME).then((imageCache) => {
          return imageCache.keys().then((requests) => {
            ...
            const cleanupPromises = requests.map((request) => {
              ...
              // Standardize key format: e.g., "/assets/gallery/album/img.webp"
      ...
      .catch((err) => {
        console.error('[SW] Granular activation cache sync stalled:', err);
